...
<div>
    ...
    <div id="main">
        <h2>我的业务/My Business</h2>
        <hr class="xuxian">
        <form action="CustomerServlet/getNumber" method="post">
            <table border="4" align="center" width="100%" height="70%">
                <tr>
                    <th>卡号：</th>
                    <td>
                        <%=session.getAttribute("card_id")%>
                    </td>
                </tr>
                <tr>
                    <th>待办业务：</th>
                    <td>
                        <%
                            String bname = "";
                            String bunmber = session.getAttribute("bnumber").toString();
                            switch (bunmber) {
                                case "00001": {
                                    bname = "取款";
                                    break;
                ...
                <tr>
                    <th>分配窗口：</th>
                    <td>
                        <span id="assign_wid">暂未分配</span>
                    </td>
                </tr>
                <tr>
                    <td colspan="3">
                        <input class="btn" type="button" value="确认" onclick="refresh()"/>
                    </td>
                </tr>
            </table>
        </form>
...
<script type="text/javascript">
    function refresh() {
        var hid = "<%=session.getAttribute("hid")%>";
        var assign_wid = document.getElementById("assign_wid");
        var xmlhttp;
        ...
        //建立连接
        xmlhttp.open("POST", "WindowServlet/assignWindow?hid=" + hid);
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                alert("!!!");
                document.getElementById("assign_wid").innerHTML = <%=session.getAttribute("assign_wid")%>;
                //document.getElementById("assign_wid").innerHTML = xmlhttp.responseText;
            }
        }
    }
</script>
